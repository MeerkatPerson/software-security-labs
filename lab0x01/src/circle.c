...
int main(int argc, char *argv[]) {
  ...
  char *input = argv[1];
  char *output = argv[2];
  // const char *hex_color_arg = argv[6];

  char *endptr;

  int center_x = strtol(argv[3], &endptr, 10);
  int center_y = strtol(argv[4], &endptr, 10);
  int radius = strtol(argv[5], &endptr, 10);

  /* Invalid color will be interpreted as black */
  /* NOTE: are these checks sufficient???? */
   ...
   */
  for (long long x = center_x - radius; x <= center_x + radius; x++) {
    long long y = round(center_y + sqrt((long long)radius * radius -
    ...
    y = round(center_y - sqrt((long long)radius * radius -
                              (x - center_x) * (x - center_x)));
   ...
   */
  for (long long y = center_y - radius; y <= center_y + radius; y++) {
    long long x = round(center_x + sqrt((long long)radius * radius -
    ...
    x = round(center_x - sqrt((long long)radius * radius -
                              (y - center_y) * (y - center_y)));
