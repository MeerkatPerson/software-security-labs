...
int main(int argc, char *argv[]) {
  ...
  if (height <= 0 || height >= (32767 * 2 + 1) || *end_ptr) {
    goto error;
  }

  long width = strtol(width_arg, &end_ptr, 10);

  if (width <= 0 || width >= (32767 * 2 + 1) || *end_ptr) {
    goto error;
  }
  ...
  /* Overflow checking */
  if (width > 9223372036854775807L / height || width < LONG_MIN / height) {
    goto error;
  }

  long square_width = strtol(square_width_arg, &end_ptr, 10);

  if (square_width <= 0 || *end_ptr || square_width > width ||
      square_width > height || width % square_width != 0 ||
  ...
  {
    struct pixel(*image_data)[width] =
        (struct pixel(*)[width])
            img->px; // sic: there had originally been a typo ('width' instead
                     // of 'square width')
