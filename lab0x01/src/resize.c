...
int main(int argc, char *argv[]) {
  ...
  double factor = strtod(argv[3], &pEnd);

  /* Resizing and image to 0 isn't allowed */
  if (factor <= 0) {
    goto error_usage; // don't use! :) You just execute the code under
                      // 'error_usage', breaks the control flow; breaks
                      // Turing-completeness
  }
  ...
  if ((double)height >= USHRT_MAX / factor ||
      (double)width >= USHRT_MAX / factor) {
    goto error_memory;
  }
  ...
  new_img = malloc(sizeof(struct image)); // lipsy
