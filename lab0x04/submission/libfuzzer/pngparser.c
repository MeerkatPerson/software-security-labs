...
error:
  if (chunk->chunk_data) {
    ...
    chunk->chunk_data = NULL; // THIS IS A FIX (for bug 01)
  }
...
error:
  ...
  if (output_buffer) {
    free(output_buffer);
    output_buffer = NULL; // THIS IS A FIX (for bug 01)
  }
...
struct image *convert_color_palette_to_image(png_chunk_ihdr *ihdr_chunk,
  ...
  if (plte_chunk == NULL) { // This is a fix for bug 01: invalid read when
                            // attempting to access plte_chunk->chunk data when plte_chunk is NULL
    return NULL;
  ...
  for (uint32_t idy = 0; idy < height; idy++) {
    ...
    if (inflated_buf[idy * (1 + width)]) {
      free(img->px);
      img->px = NULL; // THIS IS A FIX (for bug 01)
      free(img);
      img = NULL; // THIS IS A FIX (for bug 01)
      return NULL;
    }
...
int load_png(const char *filename, struct image **img) {
  ...
  current_chunk->chunk_data = NULL; // Fixes the bug described in README
  ...
  for (; !read_png_chunk(input, current_chunk);
    ...
    if (is_chunk_idat(current_chunk)) {
      ...
      if (idat_chunk->chunk_data) {
        ...
        idat_chunk->chunk_data = NULL; // THIS IS A FIX (for/as a result of bug 01)
      }

      free(idat_chunk);
      idat_chunk = NULL; // THIS IS A FIX (for/as a result of bug 01)
    }
  }
...
success:
  if(input) { // THIS IS A FIX (for bug 05)
    fclose(input);
  }

  if (inflated_buf) {  // THIS IS A FIX (for bug 02)
    free(inflated_buf);
    inflated_buf = NULL; 
  }
  ...
  if (deflated_buf) {
    free(deflated_buf);
    deflated_buf = NULL; // THIS IS A FIX (for bug 01)
  }
  if (current_chunk) {
    if (current_chunk->chunk_data) {
      free(current_chunk->chunk_data);
      current_chunk->chunk_data = NULL; // THIS IS A FIX (for bug 01)
    }
    free(current_chunk);
    current_chunk = NULL; // THIS IS A FIX (for bug 01)
  }

  if (plte_chunk) {
    if (plte_chunk->chunk_data) { // THIS IS A FIX (for bug 02)
      free(plte_chunk->chunk_data);
      plte_chunk->chunk_data = NULL;
    }
    free(plte_chunk); 
    plte_chunk = NULL; // THIS IS A FIX (for bug 01)
  }

  if (ihdr_chunk) {
    if (ihdr_chunk->chunk_data) { // THIS IS A FIX (for bug 02)
      free(ihdr_chunk->chunk_data);
      ihdr_chunk->chunk_data = NULL;
    }
    free(ihdr_chunk); 
    ihdr_chunk = NULL; // THIS IS A FIX (for bug 01)
  }

  // THIS IS A FIX (for bug 02)
  if (iend_chunk) {
...
error:
  if (*compressed_data) {
    ...
    *compressed_data = NULL; // THIS IS A FIX (for bug 01)
  }
  if (decompressed_data) {  // THIS IS A FIX (for bug 04)
...
int store_idat_plte(FILE *output, struct image *img, struct pixel *palette,
  ...
  // Move these to the start of the function; THIS IS A FIX (bug 04)
