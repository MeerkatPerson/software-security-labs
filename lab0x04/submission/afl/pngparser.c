...
error:
  if (chunk->chunk_data)
    ...
    chunk->chunk_data = NULL; // fix bug 00
  return 1;
}
...
struct image *convert_color_palette_to_image(png_chunk_ihdr *ihdr_chunk,
  ...
  if (plte_chunk == NULL) { // fixes bug 02
    return NULL;
  }
  ...
  for (uint32_t idy = 0; idy < height; idy++) {
    ...
    if ((idy * (1 + width)) >= inflated_size) { //  fixes bug 03
      return NULL;
    }
...
int load_png(const char *filename, struct image **img) {
  ...
  current_chunk->chunk_data = NULL; // fix bug 00
  ...
  for (; !read_png_chunk(input, current_chunk);
    ...
    if (is_chunk_idat(current_chunk)) {
      ...
      if (idat_chunk->chunk_data) {
        ...
        idat_chunk->chunk_data = NULL; // fix bug 00
      }

      free(idat_chunk);
      idat_chunk = NULL;   // fix bug 00
    }
  }
...
success:
  ...
  if (current_chunk) {
    ...
    free(current_chunk);
    current_chunk = NULL; // fix bug 00
  }

  if (plte_chunk) {
    free(plte_chunk);
    plte_chunk = NULL; // fix bug 00
  }
  if (ihdr_chunk) {
    free(ihdr_chunk);
    ihdr_chunk = NULL; // fix bug 00
  }
  return 0;
error:
  fclose(input);
...
error:
  if (*compressed_data) {
    ...
    *compressed_data = NULL; // fix bug 00
  }
  return 1;
}
...
error:
  if (compressed_data_buf) {
    ...
    compressed_data_buf = NULL; // fix bug 00
  }
  return 1;
}
